        ...
        <div class="product-grid grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-5"
            ...
            @foreach ($grouped as $product)
                ...
                <article
                    ...
                    data-cat="{{ $product->category_id }}" data-brand="{{ $product->brand_id ?? '' }}" data-name="{{ strtolower($product->name) }}"
                    ...
                    <div class="flex flex-col flex-1 p-3 sm:p-5 gap-1.5 sm:gap-2">
                        ...
                        <div class="font-['Fraunces',serif] text-sm sm:text-lg font-semibold text-[#27272a] leading-tight line-clamp-2"
                            title="{{ $product->name }}">
                            {{ $product->name }}
                        </div>

                        @if ($product->compatibility)
                            <div class="hidden sm:block text-xs sm:text-sm text-[#71717a] line-clamp-1">
                                Fits: {{ $product->compatibility }}</div>
                        @endif
