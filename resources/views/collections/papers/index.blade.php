...
<head>
    ...
    <link href="https://fonts.bunny.net/css?family=fraunces:400,500,600,700,900i&family=dm-sans:300,400,500"
        rel="stylesheet" />
    <!-- Updated CSS/JS assets for the paper category -->
...
<body>
    ...
    <div class="collection-hero">
        <h1> Papers</h1>
        <p>High-quality reams and rolls for pristine print results</p>
    </div>
    ...
    <div class="controls-bar">
        ...
        <div class="search-wrap">
            ...
            <!-- Corrected: Removed description searching. JS should strictly match data-name -->
            <input type="search" id="paper-search" placeholder="Search by paper name strictly…"
                aria-label="Search paper products">
        </div>

        {{-- Category pills (e.g., A4, A3, Photo Paper, Cardstock) --}}
        <div class="pill-group" id="cat-pills">
            ...
            @foreach ($category as $cat)
                <button class="pill" data-cat="{{ $cat->id }}"
                    onclick="filterCategory(this)">{{ $cat->name }}</button>
            @endforeach
        </div>

        {{-- Sort --}}
        <select class="sort-select" id="sort-select" aria-label="Sort paper">
